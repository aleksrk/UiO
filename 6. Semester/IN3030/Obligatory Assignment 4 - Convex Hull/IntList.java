class IntList {
  ...
  void append(IntList other, int start, int end) {
    if (len + (end - start) > data.length) {
        int newLen = Math.max(2 * len, len + 2 * (end - start));
        int[] b = new int[newLen];
        System.arraycopy(data, 0, b, 0, len);
        data = b;
    }

    System.arraycopy(other.data, start, data, len, end - start);
    len += (end - start);
  }
